...
- **`capability`** —— 可直接调用的函数或数据访问，叶子单元。导入的 **MCP 工具**正好投影成这一种。
...
token 生命期**锁定为 15 分钟**，可 cc-master workflow 一跑就**超过 24 小时**。Refresh 直接从**持久授权**以**相同作用域**重铸一个新鲜的 15 分钟 token：**不要 connection-key，不重新提示**，只受该授权自身有效期约束。agent 只保留短 token + 一个 refresh 句柄，从不保留 connection-key。（长时运行流程见 §5。）
