...
- **`.well-known` 指纹暴露（已接受的风险）：** 这份未认证的发现文档向任何本地调用方暴露网关版本 + capability 摘要清单。这是预会话发现（MCP 缺的那块）的代价；范围限定在摘要（ADR-008）——完整 schema / skill 主体 / `mcp.raw` 仍要过 PAT 门控的 handshake（已 enroll agent 的 `Bearer plx_agent_…`）。
- **两份凭据，绝不混淆：**
