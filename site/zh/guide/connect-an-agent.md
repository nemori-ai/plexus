...
- **专属 PAT**——**agent 的**持久凭据，由一次性 enroll 码（`plx_enroll_…`）**兑换一次**得来。agent 的命令
  在内部处理它——agent 从不读取、构造或出示凭据，也从不自己拼 HTTP。你在连接时选中的 **read**
  capability 成为**常驻授权**：那次选择*就是*人类批准。带副作用的 capability（**write** / **execute**）
  进入 agent 的世界但保持**逐次**——每次调用都挂起等你批准——除非你在连接时为那一项 capability 显式
  opt-in 常驻（默认关闭、弹窗确认），或之后在审批它的请求时选一个真实的信任窗口。请求任何你没选中的
  capability 会被直接拒绝。完整模型：[安全模型](/zh/architecture/security-model)。
:::
...
拒绝——没有审批卡，也不挂起。会挂起的是子集之内**带副作用**的 capability——**write** 或 **execute**
默认逐次批准（除非你在连接时为那一项 capability opt-in 了常驻），于是命令报 `grant_pending_user`，
转达网关撰写的说明，请你在控制台批准（**Approvals** 标签页）。对 write，批准时选的信任窗口会被尊重——
选 `Once` 表示每次都问你，选一个真实窗口（比如 `7 days`）就让*那一项* write 常驻：你显式的按项选择
说了算。对未 opt-in 的 execute，无论选哪个信任窗口都落定为 `Once`：
