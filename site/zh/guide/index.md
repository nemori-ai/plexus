...
握手之后，agent 收到按它的有效授权范围与开放状态过滤后的能力子集 manifest，知道有哪些操作、怎样使用。有效授权范围包括你为它选定的能力，以及你为它创建、未过期且通过当前 `connection-key` epoch 校验的有效常驻授权所涵盖的能力。网络可达和拿到这份清单，都不赋予调用权限。实际调用还需要有范围的授权，以及据此取得的 scoped token。`connection-key` 也不会出现在任何 agent 可达的路由上。把网关发布到局域网或隧道，只改变谁能连上它，不改变谁能批准调用。
