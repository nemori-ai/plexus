...
Plexus 随附一批第一方 source（Obsidian、Apple Calendar/Reminders、Things、cc-master）。**用户扩展**则是*你*——或替你行事的编码 agent——为网关补充新 capability 的方式：写一份 manifest，校验通过后在运行时安装。装好之后，它无需重启即出现在 `.well-known` 和每个 agent 的 manifest 里，在管理 UI 中归入 **Extensions** 层级，授权与调用方式和其他 capability 完全一致。
