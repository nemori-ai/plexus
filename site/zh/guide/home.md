...
在真实域名的 Cloudflare named tunnel 上端到端验证过。
...
——`.well-known`、auth 广告、一条命令的安装，下发的全是远端 agent 真能访问的 URL。其余一切（默认拒绝、
...
用你自己域名的 named tunnel 不受影响——所以它才是默认。
...
- `/.well-known/plexus` 变成**公开元数据**：capability 的 id + 标签（只有摘要——没有 schema，没有数据）。
  这层会话前的公开度是有意设计的；如果连"橱窗浏览"都嫌多，在域名前面加一层 **Cloudflare Access**（给 agent
