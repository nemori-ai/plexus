---
title: Level 1 · 全在一台 Mac 上
description: 安装 Plexus、启动网关、暴露一个 source、连接第一个 agent——在一台机器上跑通整套信任回环。
---

# Level 1 · 全在一台 Mac 上

**适合谁：** 所有刚接触 Plexus 的人。agent 和资源在**同一台 Mac** 上，什么都不出机器。先在这里
把*信任模型*学会，再把网关放到资源真正所在的地方。**人人先做这一档**；[Level 2](/zh/guide/home)
和 [Level 3](/zh/guide/fleet) 是同一套模型跑在更长的线上。

真实的端到端路径：安装 Plexus、启动网关、暴露一个 source、连接你的**第一个 agent**——让它发现
capability、拿到授权、完成 invoke。

Plexus 是**本地能力网关**，默认只绑定 `127.0.0.1`。开放到局域网、或发布到一个域名下
（[Level 2](/zh/guide/home)）都是可选项，且受 connection-key 管控——动手前先读[安全模型](/zh/architecture/security-model)。
所有状态都在 `~/.plexus/` 下。对这套模型（Connector → Source → Capability、来源、授权）还不熟？
可以先浏览[核心概念](/zh/concepts/)，也可以直接往下走，走完自然就懂。
...
Apple 的 TCC 授权。若尚未授予，Plexus 不会崩溃，会返回一条清晰、可恢复的提示。到 **系统设置 ▸ 隐私与安全性**
...
和任何 **execute capability** 都默认拒绝：调用挂起为 *pending*，请求出现在控制台的 **Approvals** 标签页，
...
你已经在一台机器上跑通了整套信任回环。接下来去哪：

- **[Level 2 · 从任何地方够到它 →](/zh/guide/home)**——把这台网关发布到一个域名下，让*另一台*机器上的
  agent（公司笔记本、云端 runner）经隧道调用这同一批 capability。同一套模型，更长的线。
- **[Level 3 · 给团队做资源池 →](/zh/guide/fleet)**——企业方向：一个常驻的中立网关挡在众多机器前面。
- **[连接一个 agent](/zh/guide/connect-an-agent)**——同一个首个 agent 流程的深入版（也含真实 `codex` agent）。
