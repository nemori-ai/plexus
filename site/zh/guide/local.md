...
**适合谁：** 所有刚接触 Plexus 的人。agent 和资源在**同一台 Mac** 上，什么都不出机器——在把网关
放到资源真正所在的地方之前，这里是*学信任模型*的最佳起点。**人人先做这一档**；[Level 2](/zh/guide/home)
和 [Level 3](/zh/guide/fleet) 是同一套模型跑在更长的线上。

真实的端到端路径：安装 Plexus、启动网关、暴露一个 source、连接你的**第一个 agent**——让它发现
capability、拿到授权、完成 invoke。

Plexus 是**本地能力网关**，默认只绑定 `127.0.0.1`（开放到局域网、或把它发布到一个域名下
（[Level 2](/zh/guide/home)）都是可选项，且受 connection-key 管控——请先读[安全模型](/zh/architecture/security-model)）。
所有状态都存放在 `~/.plexus/` 下。对这套模型（Connector → Source → Capability、来源、授权）还不熟？
可以先浏览[核心概念](/zh/concepts/)，也可以直接跟着往下走，自然就懂了。
