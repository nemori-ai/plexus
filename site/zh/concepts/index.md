...
Plexus 是一个**本地能力网关**。它跑在你的 Mac 上，**默认只绑定回环地址**——非回环绑定是可选项，需要用户确认
（通过 `network.json` 绑定局域网，以 connection-key 作为信任边界）。它给任何 AI agent 一套统一的 AI 原生协议，
...
| 层 | 中文 | 问题 | 例子 |
| --- | --- | --- | --- |
| **Connector**（连接器） | 怎么接 | Plexus *怎么*连接这一类东西？ | "Obsidian Local REST API"、"Obsidian vault（文件系统）"、"cc-master" |
...
- **Source** 是 connector 的*已配置实例*——你实际添加的那个东西。Source 是**受管的**：运行时即可
  添加、移除、启用、禁用、重新配置，**持久化**在 `~/.plexus/sources.json`，并**热重载**进鲜活的注册表，
...
agent 知道"这里有什么"，绝不是调用任何东西的权利。权限由人授予：限定范围、限定时限、随时可撤销。
...
- **`execute`**（或其他**高敏感度**）capability **永远不能**常驻：**每次使用**都要批准，上限就是 `once`
  ...
  都有一个新鲜的人类决定，所以它永远配不上 `7d`/`until-revoked` 窗口。这道上限是结构性的：管理员也
  无法把 `execute` capability 变成常驻。

所以信任窗口选择器会给 read 提供持久窗口，而 `execute` 授权按构造就是 `once`——常驻是 *capability*
的属性，不是 agent（甚至管理员）能为一项危险的 cap 覆盖掉的选项。
...
网关从来源 + 动词 + transport 推导出一个**敏感度**层级，只为叙述诚实——UI 和每个 agent 描述的是同一种风险：
...
1. **临时（逐操作）批准。** agent 在需要时请求授权；网关要么自动批准（比如第一方 read），要么替你
...
- **Plexus 描述*怎么使用这台机器*——并对使用设门。** 它补上了工具列表本身不携带的东西：会话前的**发现**层，
  agent 在认证之前就能先看清有什么；**来源 / 敏感度**，让风险清晰可读；**有范围、有时限、经人批准的授权**，
...
capability（MCP 来源无损保留，Plexus 可以往返回到原始 server）。MCP 是 Plexus 会说的诸多传输之一；Plexus
...
返回网关身份、一份**摘要** capability 列表（id + label + 来源——够你*看清有什么*，不够你*调用*）、
**auth 公示**（每个会话端点的 URL——`handshakeUrl`、`grantsUrl`、`invokeUrl`、…），以及 **enrollment
自描述**（`auth.enrollment`：如何用一次性码兑换 PAT）。agent **从这份公示里读端点 URL**，而不是硬编码
路径。这里不需要凭据，也不提供凭据——**connection-key 绝不出现在这里**（它仅限管理员）。这个公开、
自描述的暴露面就是 **Floor**（见[§5](#_5-编译模型-floor-及其投影)）。
...
**完整 manifest**——每个条目带完整的 `describe`、输入/输出 schema、所需动词、transport、默认信任窗口，
以及附着的 skill 正文。握手之后，agent *什么都知道*，却*什么都调用不了*：请求授权之前一律默认拒绝。

PAT 从哪来？agent 在第一次握手之前**兑换一次**：用管理员连接它时签发的**一次性 enroll 码**（见
[§5](#_5-编译模型-floor-及其投影)）。connection-key 是管理员/管理凭据，管的是握手的*管理员*路径——
不是 agent 出示的东西。
...
主机上的两个 agent 永不冲突，各自钉定自己的引擎版本——绝不是裸的全局 `plexus`）。它的子命令：

- **`plexus-<agentId> enroll <code>`**——兑换一次性码 → PAT → 自行保存（仅首次运行）。
- **`plexus-<agentId> list`**——**发现动词**：枚举这个 agent 的 capability，分为 **callable-now**
  （已有常驻授权）和 **needs-approval**。agent 靠它认清方向——包括 plugin 编译*之后*才暴露的 capability
  （Floor 是活的；投影只是它的缓存）。
