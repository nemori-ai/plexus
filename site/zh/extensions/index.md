...
好的 `describe` 是 agent 的相关性信号：说清它做**什么**、**何时**用、需要哪些输入。写具体——describe 含糊，capability 就无从被发现。
...
source **应当**实现按源的**健康协议**，让 capability 的实时可用性浮现出来——既出现在管理仪表盘里，也告知发现它的 agent：
...
- [ ] cli capability：裸 `bin` + `args` + `allowedBins`。local-rest capability：回环 `baseUrl` + `allowedHosts` + secret 引用。
