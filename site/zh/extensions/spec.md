...
管理面与能力清单视图里的一行摘要预览，就是 `describe` 的**第一行**（见 capability-registry 的 `toSummary`）；完整文本随 handshake manifest 交付。把第一行写成完整的句子。
...
5. skill/workflow 声明了**跨源附着**（伸到*另一个* source 的条目上）→ 默认拒绝（提示注入通道；须显式开启并经用户确认）。

**结构有效性（编写工具 / 规范层——良构的 manifest 必须满足）：**
6. 每个 `capabilities[].name` 在 manifest 内唯一、非空，形如 `<noun>.<verb>` slug（id 必须唯一；name 重复会撞出同一个 id）。
...
9. `kind:"workflow"` ⇒ 有 `members[]`；每个 `members[].id` 在注册时解析到**在场**的注册表条目；每个 `members[].verbs` ⊆ 该成员条目所需的 `grants`（ADR-012）。成员 id 悬空的 workflow 没有传递性授权目标——无效。
...
12. `route.attachSkills[]` 必须点名同一 manifest 里在场的 `kind:"skill"` 声明。
...
| **unregister** | `DELETE /extensions/:source`（已发布）—— `server.ts` 接有 `app.delete("/extensions/:source", …)`。它移除运行时注册的 source，**清除该 source 的授权**——但持久存储原样保留，管理员安装的扩展下次启动仍会重放。持久清除走 `DELETE /admin/api/extensions/:source`，它会额外把持久化的 manifest 从 `~/.plexus/extensions.json` 里删掉。教程就用这条路径拆掉扩展。 |
