...
- **管理平面统一由密钥门控。** 总括中间件 `admin.use("/api/*", requireManagementKey)`（`admin.ts:407`）对**每一条** `/admin/api/*` 数据路由都要求已核验的 `X-Plexus-Connection-Key`，读写一视同仁（`requireManagementKey`，`admin.ts:383-399`）。仅靠回环的 Host/Origin 守卫*不算*足够——任何本地进程都能发 `Host: 127.0.0.1`，网关也可能绑在 LAN 接口上。
...
### `execute → once` 的默认天花板（可自行核验）

`chooseTrustWindow`（`grant-service.ts:461-506`）是解析实际生效窗口的唯一收口。两道守卫让逐次批准的默认姿态成为结构性事实：
...
`def.kind === "once"` 这一分支是承重规则：capability 自身敏感度产出 `once` 默认值时（`execute` 正是如此），就返回 `once`——**不管请求什么，也不管这次选择有没有管理员权威**——除非拥有者在连接时把**那一对特定的 (agent, capability)** 显式开启为常驻 execute 授权（ADR-023：默认关闭、双重确认、可独立撤销）。这项开启经 `agentSubsets.isStanding` 查询，且只能由拥有者的连接流程写入（`core/admin.ts` 里 `POST /admin/api/agents/connect` 的按 capability `standing` 字段；旧 key `standingExecute` 作为兼容别名仍被接受）——agent 自己永远无法设置它。已开启的 execute 授权遵循管理员的权威窗口；没有给出窗口时常驻到撤销（`until-revoked`，策略禁用 `until-revoked` 时钳到 `7d`）。对 `read`/`write`，默认值绝不是 `once`，该子句从不触发，合法的管理员窗口得以生效。这道钳制在权威（管理员）与建议（agent）**两条**路径上都生效——每个铸造点都汇入 `chooseTrustWindow`（`grant-service.ts:666, 1119, 1476`）。

### 其他常驻授权规则

- **extension 的第一次授权挂起；复用短路。** `hasPriorApproval` **只**对常驻且未过期的授权返回 true（`grant-service.ts:394-402`，`isStandingAndUnexpired`）；`once` 或已过期的授权绝不短路。因此对 write/extension cap 的第一次请求会挂起等所有者批准，后续在作用域内的请求则无摩擦。
...
- **约束只收窄。** 带约束的常驻授权可以短路不带约束的请求或深度相等的请求，不短路更宽/不同的请求；铸出的 token 总是携带**常驻授权本身的**约束，绝不携带被拓宽的约束（`effectiveConstraint`，`grant-service.ts:428-434`）。
...
3. **常驻授权 + 活跃 token** —— `grants.revokeAllForAgent(agentId)` 移除该 agent 的持久授权（refresh 无法再铸造），**并给每一对打上墓碑**（仍在运行的 agent 再次请求时会重新找人类确认，而不是把低风险读再静默自动放行一次），最后撤销剩余的被追踪 jti（`revokeAllForAgent`，`grant-service.ts:1396`）。
...
- **加固的 `.well-known` 是核验基准。** 构建时校验器（`integration/verify-plugin.ts`）沿五条独立的轴，把渲染出的 plugin 对着 Floor 检查，返回结构化的通过/失败：
  ...
  5. **正文完整性** —— 手写的 skill 正文经哈希锚定（`SKILL_BODY_SHA256_PIN`）：源文必须哈希到锚定值，渲染出的 `SKILL.md` 必须逐字携带该正文，因此对教学外壳的任何改动都要先经过刻意的重新审查并重新锚定，才能出货。
...
- **隧道 auth 是双向、密钥固定的 Ed25519，失败即关闭。** proxy↔primary 边界与 agent↔primary 边界彼此分离。加入时，一次性 join token（即 nonce，静态 sha256，单次使用）准入 workload 并把它的 Ed25519 公钥**固定**下来（`mesh/enrollment.ts` 头部 + `admit`）。此后每个 socket 都跑双向挑战——primary 对着固定的密钥核验 proxy，proxy 对着固定的 `upstream.primaryPubKey` 核验 primary（强制，绝不退回 TOFU）——未 enroll/未认证的 socket 在任何数据帧之前就被断开（`mesh/handshake.ts:399-454`）。
...
一次盲测发现：含糊的 auth 错误会*诱使*谨慎的 agent 去"到磁盘上找签名密钥、铸造自己的 token"。已提交的错误响应设计把**指向受认可路径的可读性当作安全控制**：
...
已对提交的 auth 主干 + 管理/撤销路径跑过两轮对抗性红队评审。结论：**主干是干净的。** 唯一确认的 **HIGH**——`execute` capability 曾被允许在管理员给定的窗口下、且没有任何拥有者决定的情况下搭乘常驻授权——**已修复**；修复就是 `chooseTrustWindow` 里的 `def.kind === "once"` 钳制（`grant-service.ts:484`），在权威与建议两条路径上都生效。ADR-023 保留这道钳制作为默认，并加入唯一受认可的解除方式：拥有者按 (agent, capability) 逐项、在连接时双重确认的常驻 execute 开启。
