---
title: "Plexus 内部如何运作"
description: "Plexus 内部结构总览：编译模型、联邦 mesh、安全主干与线上协议，以及各部分的深入阅读入口。"
---
# 内部如何运作 {#内部如何运作}
...
### 联邦 mesh——一个入口，多个归属 {#联邦-mesh——一个入口-多个归属}
...
### 安全模型——两条信任边界 {#安全模型——两条信任边界}

信任边界恰好两条，各由一方持有。**connection-key** 是所有者的管理员凭据，agent 永远不持有。每个 **agent** 用**自己那份持久的、按 agent 独立的 PAT** 认证；PAT 由一次性 enroll 码兑换一次得来，所以 agent 凭据即使泄漏，爆炸半径也只是那一个 agent 预先获授的 capability——且可独立撤销。

敏感操作默认逐次批准。已有且仍符合条件的常驻授权可以直接使用，不必每次重新询问。接入时选中的读取能力会获得常驻授权，`write` 和 `execute` 则默认逐次批准。

所有者可为特定 agent 与 capability 显式选择常驻授权。对于 `write`，所有者也可以在批准待批请求时设定有效信任窗口，或通过显式直接授权，让它获得常驻授权。

运行代码的 `execute` 必须由所有者在接入时，为特定 agent + capability 显式开启常驻授权选项。这个选项默认关闭，开启须双重确认。没有这一步，agent 自己无论怎样请求，都解除不了逐次批准的下限。

→ [安全模型](/zh/architecture/security-model) —— 权威的信任与授权模型，条条引用到代码。授权模型的*演进方向*——任务门票、企业级归因、可插拔策略——记录在[授权可扩展性](/zh/architecture/extensibility)（ADR-020）中，尚未交付。这里的任务门票不是已有后端支持的任务包（task-bundle）。
