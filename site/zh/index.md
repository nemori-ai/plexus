...
hero:
  ...
  text: 能力网关
  tagline: >-
    把你的本地工具——笔记、日历、工作区——暴露给 AI agent，调用一律经过默认拒绝、全程审计的边界，而不是把原始密钥直接交出去。
  actions:
    - theme: brand
      text: 快速上手
      link: /zh/guide/
...
features:
  ...
  - title: 先自描述，再编译
    details: >-
      公开的 Floor 公示每项被暴露的 capability、它的输入形状和用法，任何 agent 都能读到。对有原生惯用法的 agent，Plexus 会为它编译一个专属 plugin——那只是 Floor 的一层投影，用起来像原生，却从不取代事实源。
  - title: 两种凭据，绝不混淆
    details: >-
