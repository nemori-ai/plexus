 ...
 * Keep byte-identical to the original EXCEPT two site deltas (re-copy and re-apply
 * both when the admin engine changes):
 *   1. refreshPalette() — reads the docs-site tokens (--vp-c-* / --plx-*) and
 *      detects theme via the `.dark` class instead of the admin's data-theme attr.
 *   2. layout(): agent-column x is clamped (see "SITE DELTA 2") so labels never
 *      clip in the docs-site panel, which is narrower than the admin's viewport.
 */
/**
...
export class RealtimeEngine {
  ...
  private layout(): void {
    ...
    this.agents.forEach((a, i) => {
      // SITE DELTA 2: clamp the agent column so right-aligned labels never clip at
      // the canvas edge on sub-1200px panels (admin runs full-viewport; the docs-site
      // panel does not). Floor fits the longest roster label; ceiling keeps a trail lane.
      a.tx = Math.min(Math.max(W * 0.085, 118), this.wallX - 40);
