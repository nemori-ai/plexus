---
title: Level 1 · Everything on one Mac
description: Install Plexus, start the gateway, expose a source, and connect your first agent — the whole trust loop on a single machine.
---

# Level 1 · Everything on one Mac

**Who this is for:** anyone new to Plexus. The agent and the resources live on the **same
Mac**, so nothing leaves the machine — the perfect place to *learn the trust model* before
you put the gateway where your resources actually are. **Everyone starts here**; Levels
[2](/guide/home) and [3](/guide/fleet) are the same model on a wider wire.
...
to the LAN — or publishing it under a hostname ([Level 2](/guide/home)) — is opt-in and
connection-key gated (read [the security model](/architecture/security-model) first). All
state lives under `~/.plexus/`. New to the model (Connector → Source → Capability,
...
You've run the whole trust loop on one machine. Where to go from here:

- **[Level 2 · Reach it from anywhere →](/guide/home)** — publish this gateway under a
  hostname so your agent on *another* machine (office laptop, a cloud runner) can call these
  same capabilities over a tunnel. Same model, longer wire.
- **[Level 3 · A resource pool for a fleet →](/guide/fleet)** — the enterprise direction:
  an always-on neutral gateway fronting many machines.
- **[Connect an agent](/guide/connect-an-agent)** — the same first-agent flow, in depth (a
  real `codex` agent too).
