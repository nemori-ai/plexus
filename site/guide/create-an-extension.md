...
::: tip Prerequisites
A running gateway (see [Get running](/guide/)) and the `plexus` CLI reachable. The CLI
auto-reads the connection-key from `~/.plexus/connection-key`. If you wired Codex/CC,
`plexus` is already on PATH; otherwise run the shared CLI directly with
`bun run packages/cli/src/bin/plexus <args>`. The full manifest contract is the
[authoring guide](/extensions/) and the schema reference is [the spec](/extensions/spec).
:::
...
by default — a non-loopback host is opt-in and requires an explicit, user-confirmed
`allowedHosts` entry (the approval surface). A federated multi-host topology is a
...
`/admin` before it goes live. That's the flow the acceptance harness exercises:
[`tests/harnesses/acceptance/README.md`](https://github.com/nemori-ai/plexus/blob/main/tests/harnesses/acceptance/README.md)
(a codex agent authors a vault-WRITE extension, it pends, the user approves, then it's
invoked). The `plexus extension add` path above skips the pend precisely *because* the
CLI is the admin/human surface, not an agent.
:::
