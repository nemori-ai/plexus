...
you. **Execution never stands.** An `execute` capability is approved **per use, every time** — it can
never become a standing grant, not even by you. Handing another agent the ability to *run code on
your machine* is the sharpest edge Plexus governs, so the gate stays in front of every single call.

You can see the whole rule in the approval card:

![The Plexus approval card for an execute call. Header "Grant request", tags GRANT / ORCHESTRATOR / PLEXUS-CLI, then two badges specific to this kind of call — FIRST-PARTY and ELEVATED. Plexus says: "Approving lets orchestrator EXECUTE Run Claude Code (sandboxed) (first-party, elevated-sensitivity) for this one request only; revoke anytime in Plexus → Grants." Scope: claudecode.run [execute]. The agent requested Once (advisory). A warning reads "granting execute on claudecode.run is a mutating/side-effecting grant and requires a human decision". On the right, the Trust window dropdown offers only one value — "Once" — where a protected read offered up to 7 days. Approve / Deny buttons below.](/screenshots/guide/08-execute-approval.png)
...
is `Once`** — the *only* value in the dropdown. A protected read let you pick a window up to 7 days;
execute gives you no window at all. That single dropdown with one option is the visual proof of the
rule: `execute` can never ride a standing grant. Approve this call and you've authorized exactly this
...
execute capability shows up as **needs-approval** — it can't be pre-granted:

```text
  ○ claudecode.run — Run Claude Code (sandboxed) (execute)  [first-party, elevated]
      Launch headless Claude Code to do REAL coding work ... CONFINED by the macOS sandbox to ONE
      authorized directory ... you only pass a `{ prompt }` ... This is a SENSITIVE execute
...
# (owner approves — trust window is "Once"; execute can never be a standing grant)
plexus: approved — invoking 'claudecode.run'.
{
  ...
  "output": "",
  "exitCode": null,
  "reason": "record mode: the owner has not enabled real launch for this source (Plexus console → What I expose → Claude Code → Real launch), so the sandboxed command was assembled and audited but not spawned",
  "op": "run"
}
```

Note `launched: false`. Out of the box `claudecode.run` runs in **record mode**: the sandboxed
command is fully assembled, confined, and audited, but the Claude Code process is **not spawned** — so
the call **burns no model credit**. What's real in record mode is everything that matters for trust:
the enrollment, the per-use pend, the owner's decision, and the sandbox posture
(`sandboxed: true`, the jail, `sandbox-exec`). You can walk the entire authorization loop and never
...
invoke claudecode.run detail = {
  ...
  launched: false,
  profile: ".../claudecode/sandbox/cc-confine.sb",
  argv: ["/usr/bin/sandbox-exec","-f","<profile>","-D","JAIL=<dir>", ...],   # prompt is masked to «prompt» in the audit argv
  confinement: {...}
}
```

Same call, two projections. The agent gets the minimal result it needs to act; the owner keeps the
jail path, the seatbelt mechanism, the sandbox profile, and the resolved argv — the evidence that the
...
With real launch on, the same approved call spawns a headless Claude Code inside the same
sandbox-exec jail, and the response carries a real `launched: true`, `output`, and `exitCode` instead
of the record-mode `reason`. This **really runs Claude Code and really burns model credit** — it is an
owner decision, off by default, and every run still pends per use.
...
and `claudecode.run` takes no model parameter — its argv is `claude -p <prompt>`, with no `--model`.
...
and [the security model](/architecture/security-model) for the execute-never-standing rule in full.
