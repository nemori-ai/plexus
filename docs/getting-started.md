...
to the LAN is opt-in and connection-key gated — read [security.md](security.md) first). All
state lives under `~/.plexus/`. New to the model (Connector → Source → Capability, provenance,
grants)? Skim [concepts.md](concepts.md) — or just follow along; it'll make sense.
...
> Platform: macOS (Apple Silicon or Intel). The Apple Calendar / Reminders sources are macOS-only.
...
> Prefer a GUI? `cd packages/desktop && bun run start` boots the same gateway in an Electron
> app and injects the connection-key into the console for you. Every concept below is identical.
...
> [protocol/PLEXUS-PROTOCOL.md](protocol/PLEXUS-PROTOCOL.md) — but an agent never speaks it by
> hand; the launcher does.
