...
Run it exactly as written — the absolute path works from any working directory.
Plexus is NOT an MCP server and NOT an HTTP API you call directly — there is no
wire to configure. You already have a shell, so the whole integration is: run
that command.
