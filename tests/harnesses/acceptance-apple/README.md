...
| `things`          | read+write  | `things.todos.add` (write), `things.todos.list` (read) |
...
real macOS, no TCC permission, no `osascript`, no Calendar/Reminders/Things app, no network.
...
2. **Integrates.** `GET /.well-known/plexus` (discover — the apple-calendar / apple-reminders /
   things capabilities appear, each `provenance: first-party`, each with a `health` field) →
...
3. **Grants ("为他授权对应功能").** codex requests the grants its task needs:
   ...
   `things.todos.add` (write). The **read auto-approves** (first-party read posture); the two
...
4. **Dispatch + complete the task ("派个任务..看看完成情况").** The dispatched task is:
   ...
   `apple-reminders.reminders.create` (`"Follow up on Team sync"`) and `things.todos.add`
...
5. **Audit review ("审计一下日志").** `GET /admin/api/audit` returns the full ordered chain —
   ...
   reminders.create, things.add, the verifying lists) with outcomes. The harness asserts the
...
JXA (calendar + reminders) and the Things URL-scheme (`things:///add`). The **first live use**
...
Reminders / Things and is **not** hermetic — run it deliberately, by hand, on a Mac you own.
